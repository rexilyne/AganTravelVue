<template>
    <v-main class="orderpesawat">
        <v-card>
            <v-card-title class="text-h5 justify-center font-weight-bold">Order Pesawat</v-card-title>
            <v-card-title>
                <v-text-field
                    v-model="search"
            ...
            <v-data-table :headers="headers" :items="orderPesawatArray" :search="search" :loading="load" overflow-auto>
        ...
        <v-dialog v-model="dialog" persistent max-width="600px">
            <v-card>
                ...
                <v-card-text>
                    <v-container>
                        <v-text-field v-model="form.noPenerbangan" label="No Penerbangan" :disabled="disableField"></v-text-field>
       ...
       methods: {
           ...
           readData() {
               ...
               this.$http.get(url, {
                   ...
                   this.orderPesawatArray = response.data.data;
                   this.load = false;
               })
           },
           // Simpan data orderpesawat
           save() {
               this.orderpesawat.append('noPenerbangan', this.form.noPenerbangan);
               this.orderpesawat.append('asal', this.form.asal);
               this.orderpesawat.append('tujuan', this.form.tujuan);
               this.orderpesawat.append('waktuBerangkat', this.form.waktuBerangkat);
               this.orderpesawat.append('waktuTiba', this.form.waktuTiba);
               this.orderpesawat.append('harga', this.form.harga);
               this.orderpesawat.append('idUser', localStorage.getItem('id'));
               this.orderpesawat.append('jumlahPenumpang', this.form.jumlahPenumpang);
               this.orderpesawat.append('totalHarga', this.form.totalHarga);

               var url = this.$api + '/orderpesawat'
               this.load = true;
               this.$http.post(url, this.orderpesawat, {
                   headers: {
                       'Authorization' : 'Bearer ' + localStorage.getItem('token')
                   }
               }).then(response => {
           ...
           update() {
               ...
               var url = this.$api + '/orderpesawat/' + this.editId;
               this.load = true;
           ...
           // Hapus data produk
           deleteData() {
               var url = this.$api + '/orderpesawat/' + this.deleteId;
               this.load = true;
           ...
           editHandler(item) {
               this.inputType = 'Ubah';
               this.editId = item.id;
               this.form.noPenerbangan = item.noPenerbangan;
